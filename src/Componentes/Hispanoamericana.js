...
const Hispanoamericana = (props) => {

    const [data, setData] = useState([]);
   
    useEffect(()=>{
        fetch("/hispanoamericana")
        .then(response => response.json())
        .then(function(data){
        setData(data)
    })
    }, []);
    const librosHispanoamericana = data.map(libro =>{
        return (
            <>
                  <Container>
    ...
    return (
        <>
        <h3 className="sinestilo fondo-seccion">Narrativa hispanoamericana</h3>
        <div>{librosHispanoamericana}</div>
...
export default Hispanoamericana;